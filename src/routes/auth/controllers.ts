...
export const login: RequestHandler = async (req, res, next) => {
    ...
    if (!user || !user.active)
        return next(
            new HttpException(HttpStatusCode.NOT_FOUND, '존재하지 않는 계정'),
        )
...
export const refreshToken: RequestHandler = async (req, res, next) => {
    ...
    if (!access_token || !refresh_token)
        return next(
            new HttpException(HttpStatusCode.UNAUTHORIZED, 'TOKEN FAILED'),
        )

    try {
        const access_verify = verify(access_token, {
            ignoreExpiration: true,
        }) as any
        const refresh_verify = refreshVerify(refresh_token, '')
        if (!access_verify || !refresh_verify)
            return next(
                new HttpException(HttpStatusCode.UNAUTHORIZED, 'EXPIRED TOKEN'),
            )

        const user = await User.findById(access_verify?.id)
        if (!user)
            return next(new HttpException(HttpStatusCode.NOT_FOUND, '404'))
        const new_token = sign(payload(user))

        const result: DataResponse = {
            msg: 'OK',
            data: { accessToken: new_token, refreshToken: refresh_token },
        }
        return res.json(result)
    } catch (_e) {}
